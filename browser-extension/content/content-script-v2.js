...
let processedContainers = new WeakSet(); // 记录已处理的容器
...
function tryAddMimicButtonToImage(imgElement) {
  ...
  // 查找合适的容器
  let container = findSuitableContainer(imgElement);
  if (!container) return;
  ...
  buttonWrapper.className = 'sparkit-mimic-overlay';
  ...
  // 点击事件
  buttonWrapper.querySelector('.sparkit-mimic-btn-inline').addEventListener('click', (e) => {
    e.preventDefault();
    e.stopPropagation();
...
// 查找合适的容器（图片的父元素或特定容器）
function findSuitableContainer(imgElement) {
  let container = imgElement.parentElement;
  
  // Pinterest 特殊处理：查找 pin 容器
  if (window.location.hostname.includes('pinterest')) {
    // 向上查找直到找到合适的容器
    let current = imgElement;
    while (current && current !== document.body) {
      const classes = current.className || '';
      // Pinterest 的 pin 容器通常有特定的 data 属性
      if (current.hasAttribute('data-test-id') || 
          classes.includes('Pin') || 
          current.querySelector('img') === imgElement) {
        return current;
      }
