...
export default function TasksDashboardPage() {
  ...
  const renderMediaPreview = (url: string | null, type: "image" | "video") => {
    if (!url) {
    ...
    return (
      <video
        src={url}
        controls
        className="w-32 h-24 rounded-lg border border-gray-200 bg-black"
      />
    );
  };
  ...
  if (!isAuthenticated) {
    return (
      <div className="p-10">
        <div className="rounded-xl border border-dashed border-primary-300 bg-primary-50 px-6 py-8 text-center text-primary-700">
          请先登录后再查看任务记录。
        </div>
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto p-6 lg:p-8 space-y-8">
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm space-y-6">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">任务 ID</label>
            <input
                ...
                {tasks.map((task) => (
                  <tr key={task.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      {renderMediaPreview(task.input_image_url, "image")}
                    </td>
                    <td className="px-4 py-3 text-center">
                    ...
                    <td className="px-4 py-3 text-center">
                      {renderMediaPreview(task.output_image_url, "image")}
                    </td>
                    <td className="px-4 py-3 text-center">
                      {renderMediaPreview(task.output_video_url, "video")}
                    </td>
                  </tr>
                ))}
              </tbody>
