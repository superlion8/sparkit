      ...
      if (isMaxTokens) {
        ...
        try {
          ...
          if (jsonMatch) {
            ...
            try {
              characterData = JSON.parse(incompleteJson);
          ...
          if (jsonMatch) {
            try {
              ...
              throw new Error(`无法解析JSON格式的角色描述。原因: ${finishReason}。错误: ${parseError.message}。原始响应预览: ${text.substring(0, 300)}`);
            }
          } else {
            throw new Error(`无法解析JSON格式的角色描述。原因: ${finishReason}。错误: ${parseError.message}。原始响应预览: ${text.substring(0, 300)}`);
          }
        ...
        if (jsonMatch) {
          try {
            ...
            throw new Error(`无法解析JSON格式的角色描述。错误: ${parseError.message}。原始响应预览: ${text.substring(0, 300)}`);
          }
        } else {
          throw new Error(`无法解析JSON格式的角色描述。错误: ${parseError.message}。原始响应预览: ${text.substring(0, 300)}`);
        }
